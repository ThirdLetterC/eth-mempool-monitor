...
- Required order pattern:
  ...
  2. `#include <wolfssl/openssl/...>`
- This prevents missing API/type macro definitions and build failures under `-Werror`.

### RabbitMQ BIO Type Compatibility
- In wolfSSL builds, treat `BIO_METHOD` as mutable for `BIO_new(...)` call sites.
- Use `typedef BIO_METHOD *BIO_METHOD_PTR;` (not `const BIO_METHOD *`) in `include/rabbitmq/amqp_openssl_bio.h`.
