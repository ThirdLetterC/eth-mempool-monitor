...
static ssize_t amqp_ssl_socket_send(void *base, const void *buf, size_t len,
  ...
  int res;
  ulog_trace("[rabbitmq-ssl] send request fd=%d len=%zu", self->sockfd, len);
  if (-1 == self->sockfd) {
  ...
  if (0 >= res) {
    ...
    ulog_debug("[rabbitmq-ssl] SSL_write failed fd=%d ssl_result=%d "
               "ssl_error=%d mapped_status=%d",
               self->sockfd, write_result, self->internal_error, res);
  } else {
    self->internal_error = 0;
    ulog_trace("[rabbitmq-ssl] SSL_write sent=%d fd=%d", res, self->sockfd);
  }
...
static ssize_t amqp_ssl_socket_recv(void *base, void *buf, size_t len,
  ...
  struct amqp_ssl_socket_t *self = (struct amqp_ssl_socket_t *)base;
  int received;
  ulog_trace("[rabbitmq-ssl] recv request fd=%d len=%zu", self->sockfd, len);
  if (-1 == self->sockfd) {
    return AMQP_STATUS_SOCKET_CLOSED;
  }
  ...
  if (0 >= received) {
    ...
    ulog_debug("[rabbitmq-ssl] SSL_read failed fd=%d ssl_result=%d "
               "ssl_error=%d mapped_status=%d",
               self->sockfd, read_result, self->internal_error, received);
  } else {
    ulog_trace("[rabbitmq-ssl] SSL_read received=%d fd=%d", received,
               self->sockfd);
  }
...
static int amqp_ssl_socket_open(void *base, const char *host, int port,
  ...
  X509 *cert;
  BIO *bio;
  ulog_debug("[rabbitmq-ssl] open host=%s port=%d verify_peer=%d "
             "verify_hostname=%d",
             host, port, self->verify_peer, self->verify_hostname);
  if (-1 != self->sockfd) {
  ...
  if (AMQP_STATUS_OK != status) {
    ulog_debug("[rabbitmq-ssl] amqp_time_from_now failed status=%d", status);
    return status;
  }
...
start_connect:
  ...
  if (status != 1) {
    ...
    switch (self->internal_error) {
    case SSL_ERROR_WANT_READ:
      ulog_trace("[rabbitmq-ssl] SSL_connect wants read fd=%d", self->sockfd);
      status = amqp_poll(self->sockfd, AMQP_SF_POLLIN, deadline);
      break;
    case SSL_ERROR_WANT_WRITE:
      ulog_trace("[rabbitmq-ssl] SSL_connect wants write fd=%d", self->sockfd);
...
exit:
  if (status == AMQP_STATUS_OK) {
    ulog_debug("[rabbitmq-ssl] open succeeded host=%s port=%d fd=%d", host,
               port, self->sockfd);
  } else {
    ulog_debug("[rabbitmq-ssl] open failed host=%s port=%d status=%d "
               "internal_error=%d",
               host, port, status, self->internal_error);
  }
  return status;
...
static int amqp_ssl_socket_close(void *base, amqp_socket_close_enum force) {
  struct amqp_ssl_socket_t *self = (struct amqp_ssl_socket_t *)base;
  ulog_debug("[rabbitmq-ssl] close fd=%d force=%d", self->sockfd, (int)force);
  ...
  if (amqp_os_socket_close(self->sockfd)) {
    ulog_debug("[rabbitmq-ssl] close failed fd=%d", self->sockfd);
    return AMQP_STATUS_SOCKET_ERROR;
  }
  self->sockfd = -1;

  ulog_trace("[rabbitmq-ssl] close complete");
...
static void amqp_ssl_socket_delete(void *base) {
  ...
  if (self) {
    ulog_trace("[rabbitmq-ssl] delete socket=%p", (void *)self);
...
amqp_socket_t *amqp_ssl_socket_new(amqp_connection_state_t state) {
  ...
  int status;
  ulog_trace("[rabbitmq-ssl] creating ssl socket");
  if (!self) {
    ulog_debug("[rabbitmq-ssl] allocation failed");
    return nullptr;
  }
  ...
  if (status) {
    ulog_debug("[rabbitmq-ssl] initialize_ssl_and_increment_connections "
               "failed status=%d",
               status);
    goto error;
  }
  ...
  if (!self->ctx) {
    ulog_debug("[rabbitmq-ssl] SSL_CTX_new failed");
    goto error;
  }
  status = amqp_ssl_socket_set_ssl_versions((amqp_socket_t *)self, AMQP_TLSv1_2,
                                            AMQP_TLSvLATEST);
  if (status != AMQP_STATUS_OK) {
    ulog_debug("[rabbitmq-ssl] amqp_ssl_socket_set_ssl_versions failed "
               "status=%d",
               status);
    goto error;
  }
  ...
  ulog_debug("[rabbitmq-ssl] socket created socket=%p", (void *)self);
...
void amqp_ssl_socket_set_verify_peer(amqp_socket_t *base,
  ...
  ulog_trace("[rabbitmq-ssl] set verify_peer=%d", (int)verify);
}
...
void amqp_ssl_socket_set_verify_hostname(amqp_socket_t *base,
  ...
  ulog_trace("[rabbitmq-ssl] set verify_hostname=%d", (int)verify);
}
...
int amqp_ssl_socket_set_ssl_versions(amqp_socket_t *base,
  ...
  self = (struct amqp_ssl_socket_t *)base;
  ulog_trace("[rabbitmq-ssl] set ssl versions min=%d max=%d", (int)min,
             (int)max);
  ...
  ulog_debug("[rabbitmq-ssl] configured TLS min_ver=%d max_ver=%d", min_ver,
             max_ver);
  return AMQP_STATUS_OK;
}
...
static int initialize_ssl_and_increment_connections() {
  ...
  if (!openssl_bio_initialized) {
    ...
    if (status) {
      ulog_debug("[rabbitmq-ssl] amqp_openssl_bio_init failed status=%d",
                 status);
      goto exit;
    }
    openssl_bio_initialized = 1;
    ulog_trace("[rabbitmq-ssl] OpenSSL BIO initialized");
  }

  openssl_connections += 1;
  ulog_debug("[rabbitmq-ssl] active ssl connections=%d", openssl_connections);
  status = AMQP_STATUS_OK;
exit:
  CHECK_SUCCESS(pthread_mutex_unlock(&openssl_init_mutex));
  return status;
...
static int decrement_ssl_connections() {
  ...
  if (openssl_connections == 0) {
    ...
    ulog_trace("[rabbitmq-ssl] OpenSSL BIO destroyed");
  }
