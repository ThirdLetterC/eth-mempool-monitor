...
int amqp_decode_method(amqp_method_number_t methodNumber, amqp_pool_t *pool,
  ...
  case AMQP_CONNECTION_TUNE_OK_METHOD: {
    amqp_connection_tune_ok_t *m = (amqp_connection_tune_ok_t *)amqp_pool_alloc(
  ...
  case AMQP_CONNECTION_OPEN_OK_METHOD: {
    amqp_connection_open_ok_t *m = (amqp_connection_open_ok_t *)amqp_pool_alloc(
  ...
  case AMQP_CONNECTION_BLOCKED_METHOD: {
    amqp_connection_blocked_t *m = (amqp_connection_blocked_t *)amqp_pool_alloc(
  ...
  case AMQP_EXCHANGE_DELETE_OK_METHOD: {
    amqp_exchange_delete_ok_t *m = (amqp_exchange_delete_ok_t *)amqp_pool_alloc(
  ...
  case AMQP_EXCHANGE_UNBIND_OK_METHOD: {
    amqp_exchange_unbind_ok_t *m = (amqp_exchange_unbind_ok_t *)amqp_pool_alloc(
  ...
  case AMQP_QUEUE_PURGE_METHOD: {
    amqp_queue_purge_t *m =
        (amqp_queue_purge_t *)amqp_pool_alloc(pool, sizeof(amqp_queue_purge_t));
    if (m == nullptr) {
      return AMQP_STATUS_NO_MEMORY;
    }
    if (!amqp_decode_16(encoded, &offset, &m->ticket))
  ...
  case AMQP_TX_ROLLBACK_METHOD: {
    amqp_tx_rollback_t *m =
        (amqp_tx_rollback_t *)amqp_pool_alloc(pool, sizeof(amqp_tx_rollback_t));
    if (m == nullptr) {
      return AMQP_STATUS_NO_MEMORY;
    }
    *decoded = m;
...
int amqp_decode_properties(uint16_t class_id, amqp_pool_t *pool,
  ...
  case 20: {
    amqp_channel_properties_t *p = (amqp_channel_properties_t *)amqp_pool_alloc(
  ...
  case 85: {
    amqp_confirm_properties_t *p = (amqp_confirm_properties_t *)amqp_pool_alloc(
