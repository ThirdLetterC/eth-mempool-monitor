...
[[nodiscard]] toml_datum_t toml_seek(toml_datum_t table,
  ...
  ulog_trace("[toml] seek key=%s",
             multipart_key != nullptr ? multipart_key : "(null)");
  ...
  if (!q && datum.type == TOML_TABLE) {
    auto result = toml_get(datum, p);
    ulog_trace("[toml] seek result type=%d", (int)result.type);
    return result;
  }

  ulog_trace("[toml] seek result type=%d", (int)DATUM_ZERO.type);
  return DATUM_ZERO;
}
...
void toml_free(toml_result_t result) {
  ulog_trace("[toml] free result ok=%s", result.ok ? "true" : "false");
...
toml_result_t toml_parse_file_ex(const char *fname) {
  ...
  ulog_trace("[toml] parse_file_ex path=%s",
             fname != nullptr ? fname : "(null)");
  FILE *fp = fopen(fname, "r");
  if (!fp) {
    snprintf(result.errmsg, sizeof(result.errmsg), "fopen: %s", fname);
    ulog_debug("[toml] parse_file_ex fopen failed path=%s", fname);
    return result;
  }
  result = toml_parse_file(fp);
  fclose(fp);
  if (!result.ok) {
    ulog_debug("[toml] parse_file_ex failed path=%s error=%s", fname,
               result.errmsg);
  } else {
    ulog_trace("[toml] parse_file_ex succeeded path=%s", fname);
  }
  return result;
}
...
toml_result_t toml_parse_file(FILE *fp) {
  ...
  top = max = 0;
  ulog_trace("[toml] parse_file start");
  ...
  while (!feof(fp)) {
    ...
    if (top == max) {
      ...
      if (tmpmax < 0) {
        ...
        if (max < INT_MAX - 1) {
          ...
          ulog_debug("[toml] parse_file failed: %s", result.errmsg);
          FREE(buf);
          return result;
        }
      }
    ...
    if (ferror(fp)) {
      ...
      ulog_debug("[toml] parse_file read failed: %s", result.errmsg);
      FREE(buf);
      return result;
    }
  }
  buf[top] = 0; // NUL terminator
  ulog_trace("[toml] parse_file bytes=%d", top);

  result = toml_parse(buf, top);
  FREE(buf);
  if (!result.ok) {
    ulog_debug("[toml] parse_file parse failed: %s", result.errmsg);
  } else {
    ulog_trace("[toml] parse_file parse succeeded");
  }
  return result;
}
...
toml_result_t toml_parse(const char *src, int len) {
  ...
  parser_t parser = {0};
  parser_t *pp = &parser;
  ulog_trace("[toml] parse start bytes=%d", len);
  ...
  ulog_trace("[toml] parse succeeded");
  return result;
...
bail:
  ...
  ulog_debug("[toml] parse failed: %s", result.errmsg);
  return result;
}
