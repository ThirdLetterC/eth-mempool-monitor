...
// Return filename component from a path (supports Unix and Windows separators).
[[nodiscard]] static const char *path_basename(const char *path) {
  if (path == nullptr) {
    return nullptr;
  }
...
bool color_config_is_enabled() { return color_cfg.enabled; }
...
bool prefix_config_is_enabled() { return prefix_cfg.enabled; }
...
bool time_config_is_enabled() { return time_cfg.enabled; }
...
bool level_config_is_short() { return level_cfg.short_style; }
...
bool topic_config_is_enabled() { return topic_cfg.enabled; }
...
static topic_t *topic_get_first() { return topic_data.topics; }

static topic_t *topic_get_next(topic_t *t) { return t->next; }
...
bool src_loc_config_is_enabled() { return src_loc_cfg.enabled; }
...
static void log_print_message(print_target *tgt, ulog_event *ev) {
  ...
  if (src_loc_config_is_enabled() && ev->file != nullptr) {
    print_to_target(tgt, "%s:%d: ", path_basename(ev->file),
                    ev->line); // file and line
  }
#endif // ULOG_HAS_SOURCE_LOCATION
