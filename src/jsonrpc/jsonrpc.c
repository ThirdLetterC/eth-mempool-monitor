...
static void jsonrpc_conn_ensure_arena(jsonrpc_conn_t *conn) {
  ...
  if (conn->arena != nullptr) {
    ulog_trace("[jsonrpc] arena created size=%zu conn=%p", JSONRPC_ARENA_BYTES,
               (void *)conn);
  } else {
    ulog_debug("[jsonrpc] arena create failed conn=%p", (void *)conn);
  }
}
...
static bool jsonrpc_send_value(jsonrpc_conn_t *conn, const JSON_Value *value) {
  if (conn == nullptr || conn->transport.send_raw == nullptr ||
      value == nullptr) {
    ulog_debug("[jsonrpc] send_value rejected conn=%p transport=%p value=%p",
               (void *)conn,
               conn != nullptr ? (void *)conn->transport.send_raw : nullptr,
               (void *)value);
    return false;
  }
  ...
  if (serialized == nullptr) {
    ulog_debug("[jsonrpc] serialize failed conn=%p", (void *)conn);
    return false;
  }

  const size_t len = strlen(serialized);
  ulog_trace("[jsonrpc] sending payload bytes=%zu conn=%p", len + 1U,
             (void *)conn);
  auto payload = (uint8_t *)jsonrpc_arena_malloc(len + 1U);
  if (payload == nullptr) {
    ulog_debug("[jsonrpc] payload allocation failed bytes=%zu conn=%p",
               len + 1U, (void *)conn);
...
static JSON_Value *jsonrpc_process_object(jsonrpc_conn_t *conn,
  ...
  if (obj == nullptr) {
    ulog_debug("[jsonrpc] invalid request: top-level value is not object");
  ...
  if (!has_id) {
    if (conn->callbacks.on_notification != nullptr) {
      ...
      ulog_trace("[jsonrpc] notification handled method=%s", method);
    } else {
      ulog_trace("[jsonrpc] notification dropped (no callback) method=%s",
                 method);
    }
    return nullptr;
  }
  ...
  if (conn->callbacks.on_request == nullptr) {
    ulog_debug("[jsonrpc] method not found (no request callback) method=%s",
               method);
  ...
  if (!handled) {
    ulog_debug("[jsonrpc] method not found method=%s", method);
  ...
  if (response.error_code != 0) {
    ulog_debug("[jsonrpc] handler returned error method=%s code=%d", method,
  ...
  if (response.result == nullptr) {
    ulog_debug("[jsonrpc] handler returned null result method=%s", method);
    return jsonrpc_build_error(id, JSONRPC_ERR_INTERNAL,
                               "Handler returned no result");
  }

  ulog_trace("[jsonrpc] request handled method=%s", method);
...
static JSON_Value *jsonrpc_process_value(jsonrpc_conn_t *conn,
                                         const JSON_Value *value) {
  if (value == nullptr) {
    ulog_debug("[jsonrpc] invalid request: null JSON value");
    return jsonrpc_build_error(nullptr, JSONRPC_ERR_INVALID_REQUEST, nullptr);
  }
  ...
  if (type == JSONArray) {
    ...
    ulog_trace("[jsonrpc] processing batch count=%zu", count);
    if (count == 0U) {
      ulog_debug("[jsonrpc] invalid request: empty batch");
      return jsonrpc_build_error(nullptr, JSONRPC_ERR_INVALID_REQUEST, nullptr);
    }
    ...
    if (response_count == 0U) {
      json_value_free(response_array_value);
      ulog_trace("[jsonrpc] batch produced only notifications");
      return nullptr;
    }
...
[[nodiscard]] jsonrpc_conn_t *jsonrpc_conn_new(jsonrpc_transport_t transport,
  ...
  if (conn == nullptr) {
    ulog_debug("[jsonrpc] connection allocation failed");
    return nullptr;
  }
  ...
  conn->inbound.cap = 0U;
  conn->arena = nullptr;
  ulog_debug("[jsonrpc] connection opened conn=%p user_context=%p",
             (void *)conn, external_context);
...
void jsonrpc_conn_free(jsonrpc_conn_t *conn) {
  ...
  ulog_debug("[jsonrpc] connection closing conn=%p", (void *)conn);
  conn->closed = true;
...
void jsonrpc_conn_feed(jsonrpc_conn_t *conn, const uint8_t *data, size_t len) {
  ...
  ulog_trace("[jsonrpc] feed bytes=%zu conn=%p", len, (void *)conn);

  jsonrpc_init_parson_allocator();

  if (!rpc_buffer_append(&conn->inbound, data, len)) {
    ulog_debug(
        "[jsonrpc] inbound buffer overflow conn=%p current=%zu append=%zu",
        (void *)conn, conn->inbound.len, len);
  ...
  while (true) {
    ...
    if (line_len == 0U) {
      ulog_trace("[jsonrpc] skipping empty line conn=%p", (void *)conn);
      rpc_buffer_consume(&conn->inbound, consume_len);
      continue;
    }

    if (line_len > MAX_MESSAGE_BYTES) {
      ulog_debug("[jsonrpc] message too large bytes=%zu conn=%p", line_len,
                 (void *)conn);
      const bool sent = jsonrpc_conn_send_error(
          conn, nullptr, JSONRPC_ERR_INVALID_REQUEST, "Request too large");
      if (!sent && conn->transport.close != nullptr) {
    ...
    ulog_trace("[jsonrpc] processing message bytes=%zu conn=%p", line_len,
               (void *)conn);

    auto line = (char *)jsonrpc_arena_malloc(line_len + 1U);
    if (line == nullptr) {
      ulog_debug("[jsonrpc] line allocation failed bytes=%zu conn=%p",
                 line_len + 1U, (void *)conn);
      const bool sent =
    ...
    if (request == nullptr) {
      ulog_debug("[jsonrpc] parse error conn=%p", (void *)conn);
      const bool sent =
    ...
    if (response != nullptr) {
      ...
      if (!sent && conn->transport.close != nullptr) {
        ulog_debug("[jsonrpc] send response failed, closing transport conn=%p",
                   (void *)conn);
        conn->transport.close(&conn->transport);
        close_connection = true;
        goto cleanup_message;
      ...
      ulog_trace("[jsonrpc] no response generated conn=%p", (void *)conn);
    }
...
[[nodiscard]] bool jsonrpc_conn_send_result(jsonrpc_conn_t *conn,
  ...
  ulog_trace("[jsonrpc] send result conn=%p", (void *)conn);
...
[[nodiscard]] bool jsonrpc_conn_send_error(jsonrpc_conn_t *conn,
  ...
  ulog_trace("[jsonrpc] send error conn=%p code=%d message=%s", (void *)conn,
             (int)code, message != nullptr ? message : "(default)");
