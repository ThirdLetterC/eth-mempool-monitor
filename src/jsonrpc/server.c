...
void server_set_callbacks(jsonrpc_callbacks_t callbacks) {
  ...
  ulog_trace("[jsonrpc-server] callbacks updated");
}
...
void server_request_shutdown() {
  ...
  ulog_info("[jsonrpc-server] shutdown requested");
...
static void transport_send_raw(jsonrpc_transport_t *self, const uint8_t *data,
  ...
  if (ctx == nullptr || uv_is_closing((uv_handle_t *)&ctx->tcp)) {
    ulog_trace("[jsonrpc-server] dropping send on closing/null context");
    return;
  }
  ...
  if (write_ctx == nullptr) {
    ulog_debug("[jsonrpc-server] write buffer allocation failed bytes=%zu",
               alloc_size);
    return;
  }
  ...
  ulog_trace("[jsonrpc-server] sending bytes=%zu", len);
  const int write_status =
      uv_write(&write_ctx->req, (uv_stream_t *)&ctx->tcp, &buf, 1, on_uv_write);
  if (write_status != 0) {
    ulog_error("[jsonrpc-server] uv_write failed: %s",
...
static void transport_close(jsonrpc_transport_t *self) {
  ...
  if (!uv_is_closing((uv_handle_t *)&ctx->tcp)) {
    ulog_debug("[jsonrpc-server] closing client transport");
...
static void on_uv_alloc(uv_handle_t *handle, size_t suggested_size,
  ...
  if (ctx->read_buffer == nullptr) {
    ...
    ulog_trace("[jsonrpc-server] read buffer allocated bytes=%zu",
               ctx->read_capacity);
  }
...
static void on_uv_read(uv_stream_t *stream, ssize_t nread,
  ...
  if (nread > 0) {
    ulog_trace("[jsonrpc-server] read bytes=%zd", nread);
    if (buf == nullptr || buf->base == nullptr) {
      ulog_debug("[jsonrpc-server] read with null buffer");
    ...
    ulog_debug("[jsonrpc-server] read error: %s", uv_strerror((int)nread));
    ctx->transport.close(&ctx->transport);
  }
}
...
static void on_new_connection(uv_stream_t *server, int status) {
  if (status < 0) {
    ulog_error("[jsonrpc-server] on_new_connection failed: %s",
               uv_strerror(status));
    fprintf(stderr, "on_new_connection failed: %s\n", uv_strerror(status));
    return;
  }
  ulog_debug("[jsonrpc-server] incoming connection");
  ...
  if (init_status != 0) {
    ulog_error("[jsonrpc-server] uv_tcp_init failed: %s",
  ...
  if (uv_accept(server, (uv_stream_t *)&ctx->tcp) == 0) {
    ...
    if (ctx->rpc == nullptr) {
      ulog_debug("[jsonrpc-server] jsonrpc_conn_new failed");
    ...
    if (read_status != 0) {
      ulog_error("[jsonrpc-server] uv_read_start failed: %s",
                 uv_strerror(read_status));
      fprintf(stderr, "uv_read_start failed: %s\n", uv_strerror(read_status));
      transport_close(&ctx->transport);
    }
  } else {
    ulog_debug("[jsonrpc-server] uv_accept failed");
    uv_close((uv_handle_t *)&ctx->tcp, on_uv_client_closed);
  }
}
...
void start_jsonrpc_server(const char *host, int32_t port, int32_t backlog,
  ...
  int run_status = 0;
  const char *bind_host = host != nullptr ? host : SERVER_DEFAULT_HOST;
  if (backlog <= 0) {
    backlog = SERVER_DEFAULT_BACKLOG;
  }
  ulog_info("[jsonrpc-server] starting host=%s port=%d backlog=%d", bind_host,
  ...
  const int addr_status = uv_ip4_addr(bind_host, (int)port, &addr);
  if (addr_status != 0) {
    ulog_error("[jsonrpc-server] uv_ip4_addr failed: %s",
  ...
  if (bind_status != 0) {
    ulog_error("[jsonrpc-server] uv_tcp_bind failed: %s",
  ...
  const int listen_status =
      uv_listen((uv_stream_t *)&g_server, (int)backlog, on_new_connection);
  if (listen_status != 0) {
    ulog_error("[jsonrpc-server] uv_listen failed: %s",
  ...
  ulog_info("[jsonrpc-server] event loop stopped status=%d", run_status);
...
cleanup_loop:
  ...
  if (loop_status != 0) {
    ulog_error("[jsonrpc-server] uv_loop_close failed: %s",
  ...
  if (run_status != 0) {
    ulog_warn("[jsonrpc-server] uv_run exited with active handles (%d)",
              run_status);
